...
public partial class NurseRequestToPharmacy_IpdPatients : System.Web.UI.Page
{
    clsEmr obj = new clsEmr();
    clsTreatment objTreat = new clsTreatment();
    clsTreatmentTransaction transaction = new clsTreatmentTransaction();
    DataTable tempDatatable = new DataTable();
    public NurseRequestToPharmacy_IpdPatients()
    {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!this.IsPostBack)
        {
            ...
            if (Session["EmrRegNo"] != "")
            {
                ...
                BindPharmacyDrugsReqByNurse(Convert.ToInt32(txtpatientregid.Text),Convert.ToInt32(IpdNo));
                BindGridviewData();
            }
            else
            {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (tempDatatable != null && tempDatatable.Rows.Count > 0)
            {
                ...
                objTreat.ipd = lblIpd.Text;
                objTreat.opd = lblOpd.Text;
                ...
                int res = 0;
                if (objTreat.getallreadyIPDRecPayment_New(DateTime.Now.AddMinutes(-1), Convert.ToInt32(Session["EmrRegNo"]), Convert.ToInt32(lblIpd.Text), Convert.ToInt32(lblOpd.Text)))
                {
                    res = objTreat.InsertUpdateDeleteMain_Ipd(ttype);
                     if (res == 0)
                     {
                         DataTable dtid=new DataTable ();
                         dtid = transaction.GetTreatmentIDFor_PatientId(Convert.ToString(Session["EmrRegNo"]), Convert.ToString(lblOpd.Text), Convert.ToString(lblIpd.Text));
                ...
                else
                {
                    ...
                    if (res > 0)
                    {
                        ...
                        BindPharmacyDrugsReqByNurse(Convert.ToInt32(txtpatientregid.Text), Convert.ToInt32(lblIpd.Text));
    ...
    {
        GvNoteIngrid.DataSource = transaction.GetReqDrugByNurse(PatRegId, IpdNo);
    ...
    {
        GvNoteIngrid.DataSource = transaction.GetTreatementDetails(PatRegId);
        GvNoteIngrid.DataBind();
        for (int i = 0; i < GvNoteIngrid.Rows.Count; i++)
        {
    ...
    //    string treatId = (gdvTreatment.DataKeys[e.NewEditIndex]["TreatmentId"].ToString());
    //    ViewState["catId"] = txtTreatId.Value = treatId;

    //    GridViewRow row = (GridViewRow)gdvTreatment.Rows[rowIndex];

    //    Label TreatmentId = (Label)row.FindControl("TreatmentId");
    ...
    //    Label TransId = (Label)row.FindControl("TransId");
    //    Label DrugName = (Label)row.FindControl("DrugName");
    //    Label FrequencyType = (Label)row.FindControl("FrequencyType");
    //    Label Days = (Label)row.FindControl("Days");
    //    Label StartDate = (Label)row.FindControl("StartDate");
    //    Label EndDate = (Label)row.FindControl("EndDate");
    //    Label Note = (Label)row.FindControl("Note");

    //    try
    //    {
    //        txtPatientRegId.Text = PatientRegId.Text;
    //        txtOpd.Text = Opd.Text;
    //        txtIpd.Text = Ipd.Text;
    //        txtBranchId.Text = BranchId.Text;
    //        txtFollowUp.Text = Convert.ToDateTime((FollowUpDate.Text).Replace('/', '-')).ToShortDateString();

    //        ViewState["TrId"] = TransId.Text;

    //        txtDrugName.Text = DrugName.Text;
    //        int index = drpfrequency.Items.IndexOf(drpfrequency.Items.FindByText(FrequencyType.Text));
    //        drpfrequency.SelectedIndex = index;

    //        txtDays.Text = Days.Text;
    //        txtStart.Text = Convert.ToDateTime((StartDate.Text).Replace('/', '-')).ToShortDateString();
    //        txtEnd.Text = Convert.ToDateTime((EndDate.Text).Replace('/', '-')).ToShortDateString();
    ...
    //protected void txtPatientRegId_TextChanged(object sender, EventArgs e)
    //{
    //    string pId = txtPatientRegId.Text;
    //    string opd = txtOpd.Text;
    //    string ipd = txtIpd.Text;
    //    BindMainGrid(pId, ipd, opd);
    //    GetTreatmentId(pId, ipd, opd);
    //}
    ...
    //protected void txtOpd_TextChanged(object sender, EventArgs e)
    //{
    //    string pId = txtPatientRegId.Text;
    //    string opd = txtOpd.Text;
    //    string ipd = txtIpd.Text;
    ...
    //private void GetTreatmentId(string pid, string ipd, string opd)
    //{
    //    DataTable dt = objTreat.CheckTreatmentDetails(pid, ipd, opd);
    //    if (dt != null)
    //    {
    //        string val = "0";
    ...
    //        if (dt.Rows.Count > 0)
    //        {
    //            val = dt.Rows[0]["TreatmentId"] != DBNull.Value ? Convert.ToString(dt.Rows[0]["TreatmentId"]) : "0";

    //            //txtOpd.Text = Convert.ToString(dt.Rows[0]["Opd"]);
    //            txtIpd.Text = Convert.ToString(dt.Rows[0]["Ipd"]);
    //            txtBranchId.Text = Convert.ToString(dt.Rows[0]["BranchId"]);
    //            txtFollowUp.Text = Convert.ToDateTime(Convert.ToString(dt.Rows[0]["FollowUpDate"])).ToShortDateString().Replace('/', '-');
    //            ViewState["CreatedBy"] = Convert.ToString(dt.Rows[0]["CreatedBy"]);
    //            ViewState["CreatedOn"] = Convert.ToString(dt.Rows[0]["CreatedOn"]);

    //            ViewState["TreatId"] = string.IsNullOrEmpty(val) ? "0" : val;
    //        }
    //    }
    //    else
    //    {
    ...
    //    txtTreatId.Value = Convert.ToString(ViewState["TreatId"]);
    //}
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            float Qty = Convert.ToSingle(0);
            tempDatatable = (DataTable)ViewState["tempDatatable"];
            if (Convert.ToInt32(ViewState["TransId"]) > 0)
            {

                transaction.UpdateTreatementTransaction_555(drugname, DoseId,Dose,DoseTimeId,Convert.ToString(days), note, ItemId, Convert.ToInt32(ViewState["TransId"]),freq,"","","");
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                for (int i = 0; i < dt.Rows.Count; i++)
                                {
                                  ...
                                  //  tempDatatable.Rows.Add(tempDatatable.Rows.Count + 1, drugname, Qty, startDate, endDate, note, ItemId);
                                    tempDatatable.Rows.Add(tempDatatable.Rows.Count + 1, drugname, Dose, freq, days, startDate, endDate, note, DoseTimeId, DoseId, ItemId);
                    ...
                    else if (btnAdd.Text == "Update")
                    {
                        ...
                        if (rowId >= 0)
                        {
                            ...
                            if (row != null)
                            {
                                ...
                                tempDatatable.Rows.Add(rowId, drugname, Dose, freq, days, startDate, endDate, note, DoseTimeId, DoseId, ItemId);
    ...
    public void Alter_view()
    {
        ...
        " where tbl_Treatment.branchid=" + Convert.ToInt32(Session["Branchid"]) + " and (tbl_Treatment.PatientRegId = " + Convert.ToString(Session["EmrRegNo"]) + ") AND (tbl_Treatment.Ipd = " + Convert.ToString(Session["EmrIpdNo"]) + ")    ";//AND (tbl_Treatment.Ipd = "+Convert.ToString(Session["EmrIpdNo"])+")
