...
public partial class DischargeMedication : System.Web.UI.Page
{
    ...
    private void BindFrequencyDrop()
    {
        ...
        ddlRoute.DataBind();
        ddlRoute.Items.Insert(0, new ListItem("--Select--", "-1"));
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (tempDatatable != null && tempDatatable.Rows.Count > 0)
            {
                ...
                objTreat.FId = Convert.ToInt32(Session["FId"]);
                objTreat.DiscMedication = true;
                objTreat.IsApproveByNurse = true;
                int res = 0, res1 = 0;
                if (objTreat.getallreadyIPDRecPayment_New_Discharge_Medi(DateTime.Now.AddMinutes(-1), Convert.ToInt32(Session["EmrRegNo"]), Convert.ToInt32(Session["EmrIpdNo"]), Convert.ToInt32(Session["EmrOpdNo"])))
                {
                    res = objTreat.InsertUpdateDeleteMain(ttype);
                    res1 = objTreat.InsertUpdateTreatmentDiagnostic(txtDiagnosis.Text);
                    if (res == 0)
                    {
                        DataTable dtid = new DataTable();
                        dtid = transaction.GetTreatmentIDFor_PatientId_DiscMedi(Convert.ToString(Session["EmrRegNo"]), Convert.ToString(Session["EmrOpdNo"]), Convert.ToString(Session["EmrIpdNo"]));
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                txtDiagnosis.Focus();
                txtDiagnosis.ForeColor = System.Drawing.Color.Red;
                txtDiagnosis.BorderColor = System.Drawing.Color.Red;
                lblMsg.Text = "Enter Diagnosis!";
                lblMsg.ForeColor = System.Drawing.Color.Red;
                lblMsg.Visible = true;
                return;
            }
            ...
            string InstName = ddlinstruction.SelectedItem.Text;
            string NewDose = txtnewDose.Text;
            string Route = ddlRoute.SelectedItem.Text;
            if (txtQty.Text != "")
            {
    ...
    public void Alter_view()
    {
        ...
        string query = "ALTER VIEW [dbo].[VW_GetPrescription] AS (SELECT        tbl_Treatment.TreatmentId, tbl_Treatment.FollowUpDate, tbl_Treatment.PatientRegId, tbl_Treatment.Opd, tbl_Treatment.Ipd, tbl_Treatment.BranchId, tbl_Treatment.CreatedBy, tbl_Treatment.CreatedOn, tbl_Treatment.UpdatedBy, " +
                      ...
                      "  tbl_TreatmentTransaction.StartDate, tbl_TreatmentTransaction.EndDate, tbl_TreatmentTransaction.Note, tbl_TreatmentTransaction.Dose ,HospEmpMst.Title+' '+ HospEmpMst.Empname as DrName, DepartmentMst.DeptName, HospEmpMst.EmployeeType , IPDDischargeSummary.FinalDiagnosis as Diagnosis, HospEmpMst.DrSignature " +
                      "  , tbl_TreatmentTransaction.InstName,tbl_TreatmentTransaction.NewDose,tbl_TreatmentTransaction.Route FROM            tbl_Treatment INNER JOIN " +
                      ...
                      "  DepartmentMst ON HospEmpMst.DeptId = DepartmentMst.DeptId LEFT OUTER JOIN " +
                      "  IPDDischargeSummary ON tbl_Treatment.PatientRegId = IPDDischargeSummary.PatRegId AND tbl_Treatment.Ipd = IPDDischargeSummary.IpdNo " +
        " where tbl_Treatment.DiscMedication=1 and  tbl_Treatment.branchid=" + Convert.ToInt32(Session["Branchid"]) + " and (tbl_Treatment.PatientRegId = " + Convert.ToString(Session["EmrRegNo"]) + ") AND (tbl_Treatment.Opd = " + Convert.ToString(Session["EmrOpdNo"]) + ")    ";//AND (tbl_Treatment.Ipd = "+Convert.ToString(Session["EmrIpdNo"])+")
    ...
    protected void FillPage()
    {
        try
        {
            ...
            ddlinstruction.SelectedItem.Text = Obj.InstName;
            txtnewDose.Text = Obj.NewDose;
