...
public class Graphs : System.Web.Services.WebService
{
    ...
    public void MenusBind()
    {
        ...
        dt = ObjTB.BindMainMenu(Convert.ToString(Session["username"]), Convert.ToString(Session["password"]));//,Convert.ToString( Session["ModuleId"])
                    ...
                    if (dtchild.Rows.Count > 0)
                    {
                        ...
                        for (int j = 0; j < dtchild.Rows.Count; j++)
                        {
                            ...
                            if (dtform.Rows.Count > 0)
                            {
                                ...
                                for (int k = 0; k < dtform.Rows.Count; k++)
                                {
                                    if (dtform.Rows[k]["SubMenuNavigateURL"].ToString() != "#")
                                    {
                                        obChild.Url = dtform.Rows[k]["SubMenuNavigateURL"].ToString().Remove(0, 2);
                                    }
                                }
    ...
    public void IPDMenusBind()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                {
                    DataTable dtchild = new DataTable();
                    dtchild = ObjTB.IPDBindChildMenu(dt.Rows[i]["MenuID"].ToString(), Convert.ToString(Session["username"]),
                        Convert.ToString(Session["password"]));
                    ...
                    if (dtchild.Rows.Count > 0)
                    {
                        ...
                        for (int j = 0; j < dtchild.Rows.Count; j++)
                        {
                            ...
                            DataTable dtform = new DataTable();
                            dtform = ObjTB.IPDBindForm(Convert.ToInt32(obChild.ChildMenuId));
    ...
    [WebMethod]
    public void Get_TotalNoOfPatients_AdmitedMonth(string month, string year, string Center)
    {
        List<cls_1> list = new List<cls_1>();

        DataTable dt = GetDataFromDataTable5("usp_Graph2_NoOfPatients_Admited", month, year, Center);
    ...
    [WebMethod]
    public void Get_TotalPatientsAdmitedByDept(string month, string year, string center)
    {
        List<cls_1> lists = new List<cls_1>();

        DataTable dt = GetDataFromDataTable4("usp_Graph1_NoOfPatients_ByIPDDept", Convert.ToInt32(month), Convert.ToInt32(year), center);
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
    ...
    [WebMethod]
    public void Get_Top5DoctorsByAdmitedPatient(string month, string year, string center)
    {
        List<cls_1> lists = new List<cls_1>();

        DataTable dt = GetDataFromDataTable4("usp_GetTop5DoctorsByAdmitedPatientRefered",
            Convert.ToInt32(month), Convert.ToInt32(year), center);
       ...
       [WebMethod]
    public void Get_TopRoomCategoryByAdmitedPatient(string month, string year, string center)
    {
        List<cls_1> lists = new List<cls_1>();

        DataTable dt = GetDataFromDataTable4("usp_GetTop5DoctorsByAdmitedRoomWise",
            Convert.ToInt32(month), Convert.ToInt32(year), center);
       ...
       [WebMethod]
       public void Get_TotalNoOfPatients_OPDMonth(string month, string year, string Center)
       {
           List<cls_1> list = new List<cls_1>();

           DataTable dt = GetDataFromDataTable5("usp_Graph2_NoOfOPDPatients", month, year, Center);
       ...
       [WebMethod]
       public void Get_TotalPatientsOPDByGroupWise(string month, string year, string center)
       {
           List<cls_1> lists = new List<cls_1>();

           DataTable dt = GetDataFromDataTable4("usp_Graph1_NoOfPatients_ByGroupWise", Convert.ToInt32(month), Convert.ToInt32(year), center);
           if (dt.Rows.Count > 0)
           {
               for (int i = 0; i < dt.Rows.Count; i++)
               {
