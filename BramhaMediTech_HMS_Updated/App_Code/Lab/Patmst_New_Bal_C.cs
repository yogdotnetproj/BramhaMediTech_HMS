...
public class Patmst_New_Bal_C
{
    ...
    public static DataTable GetPatmstForPanicPatient(object UnitCode, object DeptID, object fromDate, object toDate, string Patauthicante, string MTCode, string patientName, string regnoID, string Barid, string CenterCode, string username, string usertype, int branchid, int maindept, int Number, string testname, string MNo, string CenterCodeNew)
    {
        ...
        SqlConnection conn = new SqlConnection(ConnectionString.ConnectionstringPatho);
    ...
    public DataSet Get_Phlebotomist_MainDoc_2Way(string UnitCode, object startDate, object endDate, string patientName, string Reg_no, int branchid, string maindept, string FID, int Number, string username, string UserType, object DoctorCode, string Barcode, string NewBarcode, string OutNewBarcode)
    {
        SqlConnection conn = new SqlConnection(ConnectionString.ConnectionstringPatho);
        ...
        if (Reg_no != "")
        {
            sql = sql + " and ( PatRegID='" + Reg_no + "' or LabRegMediPro='" + Reg_no + "' )";
        }
        else
        {
            if (patientName != "")
    ...
    public DataSet Get_Phlebotomist_2Way(object DoctorCode, object startDate, object endDate, string patientName, string PatReg_ID, int branchid, string maindept, string FID, int Number, string username, string UserType, string Barcode, string Mobno, string NewBarcode, string OutNewBarcode)
    {
        SqlConnection conn = new SqlConnection(ConnectionString.ConnectionstringPatho);
        ...
        if (PatReg_ID != "")
        {
            sql = sql + " and (PatRegID='" + PatReg_ID + "' or PPID='" + PatReg_ID + "'  )";//or LabRegMediPro='" + Reg_no + "'  or Barcodeid='" + Reg_no + "'
        }
        else
        {
            if (patientName != "")
    ...
    public DataTable Get_PatientCallDetaiis_Report(string PatRegID, string fromdate, string Todate, string Call)
    {
        ...
        string sql = " SELECT   DISTINCT  c.PatRegID, c.Patregdate, c.intial, c.Patname, c.sex, c.Age, c.MDY, c.RefDr, " +
          ...
          sql = sql + "   ORDER BY c.Patregdate DESC ";
    ...
    {
        SqlConnection con = DataAccess.ConInitForPathology();
        SqlDataAdapter da = new SqlDataAdapter(" SELECT c.*, cm.AmtPaid,cm.Discount, cm.DisFlag, c.intial + ' ' + c.Patname AS FullName FROM dbo.patmst c LEFT OUTER JOIN dbo.Cshmst cm " +
            " ON c.PID = cm.PID AND c.PatRegID = cm.PatRegID where c.Patusername =  N'" + username + "'  order by c.SrNo desc ", con);
        DataTable ds = new DataTable();
        try
        {
            da.Fill(ds);
    ...
    public DataTable GetReceiptNo(int branchid, int PID)
    {
        ...
        SqlConnection conn = data.ConInitForPathology1();
    ...
    public static ICollection Get_patmst_againsPIDID(int PID, int branchid)
    {
        ...
        SqlCommand sc = new SqlCommand("SELECT        patmst.SrNo, patmst.PID, patmst.FID, patmst.PatRegID, patmst.Patregdate, patmst.intial, patmst.Patname, patmst.sex, patmst.Age, patmst.MDY, patmst.RefDr, patmst.Tests, patmst.PF, patmst.Reportdate, "+
          ...
          "  patmst.FSTime, patmst.Therapy, patmst.SocialMedia, patmst.ReportAssignStatus, PatMT.PatientCardNo, PatMT.PatientCardExpNo, PatMT.DateOfBirth, PatMT.AccDateofBirth ,patmst.UploadPrescription " +
    ...
    public static string PatientCountBanner(int branchid)
    {
        ...
        conn.Open();
        string sql = "";
        sql = sql + "Select count(*) from PatientInformation where branchid=" + branchid + "";
    ...
    public DataSet Get_patmst_Edittest(string PateintName, string PatRegID, DateTime dfrom, DateTime dto, int branchid, string CenterName, string labcode_main, string FID, string Barcode, string Mno)
    {
        ...
        if (CenterName != "")
        {
            sql = sql + " and CenterName= '" + CenterName + "'";
        ...
        else
        {
            ...
            else
            {
                if (dfrom != null && dto != null)
                {
                   ...
                   // sql = sql + " and Phrecdate between '" + Convert.ToDateTime(dfrom.ToString()).ToString("dd/MM/yyyy") + "' and '" + Convert.ToDateTime(dto.ToString()).ToString("dd/MM/yyyy") + "' ";
        ...
        if (Mno != "")
        {
            sql += " and Patphoneno like '%" + Barcode + "%'";
        }
    ...
    public DataSet ReportDownloadModify(object DoctorCode, object startDate, object endDate, object Patauthicante, string patientName, string Reg_no, int branchid, int DigModule, string subdeptName, string FID, string labcode_main, string Barcode, string Subdept, string RepStatus)
    {
        SqlConnection conn = DataAccess.ConInitForPathology();
        string sql = "Select  PatRegID, Name, sex, DoctorCode, Age, MDY,''as PatientcHistory, Drname, CenterName, Patauthicante, Maintestname, CenterCode, Phrecdate, Patname, branchid, FID, "+
           " OutStAmt, SDCode, TestCharges, Patregdate, PPID, UnitCode, BarcodeID, Patrepstatus, AmtReceived, Discount, DoctorEmail, PatientEmail,LabRegMediPro,TelNo " +
         " from VW_patrprtvwd where FID='" + FID + "' and PatRegID<>'' and branchid=" + branchid + " ";
    ...
    public DataSet getpatientbillcanceled(string Cname, object startDate, object endDate, string PatRegID, int branchid, int DigModule, string collectioncode, string FID, string UnitCode)
    {
        ...
        string query = "SELECT * ,intial+' '+ FirstName as Fullname FROM         VW_csmst1vw INNER JOIN "+
               " SaleCancelDetails ON VW_csmst1vw.PID = SaleCancelDetails.PID where VW_csmst1vw.IsbillBH=0 and VW_csmst1vw.Monthlybill=0  and VW_csmst1vw.IsActive=0  and VW_csmst1vw.PatRegID<>'' and VW_csmst1vw.branchid=" + branchid + "";//and FID='" + FID + "'
    ...
    public DataSet GetPatientInformationnew_Refundpay(string Cname, object startDate, object endDate, string PatRegID, int branchid, int DigModule, string collectioncode, string FID, string UnitCode, string Patname, string Mobno, string Centername)
    {
        ...
        {
            query += " and CenterName='" + Centername + "'";
    ...
    public DataSet GetPatientInformationnew_1(string Cname, object startDate, object endDate, string PatRegID, int branchid, int DigModule, string collectioncode, string FID, string UnitCode,string Patname,string Mobno,string Centername)
    {
        SqlConnection conn = DataAccess.ConInitForPathology();
        SqlDataAdapter da;
        DataSet ds = new DataSet();
        ...
        string query = "SELECT   PID, PatRegID, FID, Patregdate, intial, FirstName,  sex, Age, MDY, RefDr, Tests, Phrecdate, Branchid, DoctorCode,  CenterCode,  " +
         " FinancialYearID, EmailID, Drname, CenterName, Username, Usertype,  PPID, testname, AmtPaid, Discount, UnitCode, TestCharges, ROUND( balance,0) as balance,  " +
         " IsbillBH, BillNo, Patphoneno, IsActive, TaxPer, TaxAmount, FirstName as Fullname,IsbillBH from  VW_csmst1vw  where  IsActive=1  and  VW_csmst1vw.PatRegID<>'' and branchid=" + branchid + "";//IsbillBH=0 and Monthlybill=0  and
        if (Centername != null && Centername != "All")
        {
        ...
        " FinancialYearID, EmailID, Drname, CenterName, Username, Usertype,  PPID, testname, AmtPaid*-1, Discount, UnitCode, TestCharges, ROUND( balance,0) as balance,  " +
        " IsbillBH, BillNo, Patphoneno, IsActive, TaxPer, TaxAmount, FirstName as Fullname,IsbillBH from  VW_csmst1vw  where    IsActive=0  and VW_csmst1vw.PatRegID<>'' and branchid=" + branchid + "";//IsbillBH=0 and Monthlybill=0  and
        if (Centername != null && Centername != "All")
        {
            query += " and CenterName like N'%" + Centername + "%'";
    ...
    {
        SqlConnection conn = DataAccess.ConInitForPathology();
        string sql = "Select * from VW_patrprtvwd where FID='" + FID + "' and PatRegID<>'' and branchid=" + branchid + "";
    ...
    public DataTable get_PAtientInformationWithoutStatus(object CenterCode, object UnitCode, object Patauthicante, int branchid, int DigModule, string FID, int i)
    {
        ...
        if (CenterCode != null)
        {
            sql = sql + " and " + "patmst.CenterCode='" + CenterCode.ToString() + "'";
        }
        if (FID != null)
        {
    ...
    public DataSet Get_Phlebotomist(object DoctorCode, object startDate, object endDate, string patientName, string Reg_no, int branchid, string maindept, string FID, int Number, string username, string UserType, string Barcode, string Mobno, string NewBarcode)
    {
        SqlConnection conn = new SqlConnection(ConnectionString.ConnectionstringPatho);
    ...
    public DataSet Get_Phlebotomist_MainDoc(string UnitCode, object startDate, object endDate, string patientName, string Reg_no, int branchid, string maindept, string FID, int Number, string username, string UserType, object DoctorCode, string Barcode, string NewBarcode)
    {
        SqlConnection conn = new SqlConnection(ConnectionString.ConnectionstringPatho);
    ...
    public static void AlterViewvw_GroupByLabcode_New(string usertype,string Username)
    {
     ...
     public DataTable Get_Testresultentrystatus(object UnitCode, object DeptID, object fromDate, object toDate, string Patauthicante, string MTCode, string patientName, string regnoID, string vial, string CenterCode, string username, string usertype, int branchid, int maindept, int Number, string FID, string MNo)
     {
         ...
         if (CenterCode != "")
         {
             sql = sql + " and " + " VW_patlbvw.CenterCode ='" + CenterCode + "'";
         }
         if (vial != "")
         {
     ...
     public DataTable GetPatientforBill_Saleregister(string Cname, object startDate, object endDate, string PatRegID, int branchid, int DigModule, string Patname, string UserName, string UnitCode, string DoctorCode, int paid)
     {
         ...
         string query = "SELECT     PID, PatRegID, FID, Patregdate, intial, FirstName, sex, Age, FirstName, MDY, RefDr, Tests, Phrecdate, Branchid, DoctorCode, CenterCode, FinancialYearID, EmailID, " +
     ...
     public DataSet GetPatientfor_servicewise_saleReport(string Cname, object startDate, object endDate, string PatRegID, int branchid, int DigModule, string collectioncode, string FID, string UnitCode, string DoctorCode, int paid, string Maintestname)
     {
         ...
         string query = "SELECT DISTINCT "+
                       ...
                       " (SUM(patmstd.TestRate)-  dbo.get_discountwiseService_amt(patmstd.MTCode)) * Cshmst.TaxPer / 100 AS Tax, "+
                       " patmstd.MTCode,dbo.get_discountwiseService_amt(patmstd.MTCode) as Discount "+
                       " ,SUM(patmstd.TestRate)-  dbo.get_discountwiseService_amt(patmstd.MTCode)+(SUM(patmstd.TestRate)-  dbo.get_discountwiseService_amt(patmstd.MTCode)) * Cshmst.TaxPer / 100 as Net "+
                       " FROM         Cshmst INNER JOIN "+
                       " patmstd ON Cshmst.PatRegID = patmstd.PatRegID AND Cshmst.PID = patmstd.PID INNER JOIN "+
                       " MainTest ON patmstd.MTCode = MainTest.MTCode "+
                      "   where patmstd.isactive=1 and Cshmst.branchid=" + branchid + "";//
         if (UnitCode != null && UnitCode != "")
         {
             query += " and UnitCode='" + UnitCode + "'";
         ...
         if (PatRegID != "" && PatRegID != null)
         {
             query += " and Cshmst.PatRegID='" + PatRegID + "'";
         }
         else
         {
             if (Cname != "All" && Cname != "")
             {
                 query += " and CenterCode='" + Cname + "'";
             }
             if (startDate != null && endDate != null)
             {
                 query = query + " and Cshmst.RecDate between '" + Convert.ToDateTime(startDate.ToString()).ToString("dd/MMM/yyyy") + "' and '" + Convert.ToDateTime(endDate.ToString()).ToString("dd/MMM/yyyy") + "' ";
             }
         }
         query += " GROUP BY  patmstd.isactive, MainTest.Maintestname, Cshmst.TaxPer, patmstd.MTCode order by MainTest.Maintestname ";
         da = new SqlDataAdapter(query, conn);
         ds = new DataSet();
         try
         {
     ...
     public static void AlterViewvw_GroupByLabcode_New_testwise(string usertype, string Username, object startDate, object endDate, string patientName, string regnoID, string Barid, string CenterCode, string username,  int branchid, int maindept, int Number, string testname, string MNo, string CenterCodeNew)
     {
         SqlConnection conn = new SqlConnection(ConnectionString.ConnectionstringPatho);
         SqlCommand sc = conn.CreateCommand();
         ...
         if (usertype == "Main Doctor" || usertype == "Technician" || usertype == "Reporting")
         {
             ...
             sc.CommandText = "   Alter View VW_patlbvw as   SELECT     patmst.PatRegID, CAST(patmstd.BarcodeID AS varchar(255)) AS Test1, patmst.PID, patmst.FID, patmst.Patregdate, patmst.intial, "+
                  "  patmst.Patname, patmst.sex, patmst.Age,   "+
                  "  patmst.RefDr, patmst.Reportdate, patmst.DoctorCode, patmst.CenterCode, patmst.FinancialYearID, patmst.TestCharges, patmst.Username, patmst.SampleType,   "+
                  "  patmst.SampleStatus, patmst.RegistratonDateTime, patmstd.BarcodeID, patmst.Drname, patmst.MDY, CAST(patmst.PatientcHistory AS varchar(255)) AS cliniclahist,   "+
                  "  patmst.Branchid, SubDepartment.DigModule, patmstd.UnitCode, patmst.Remark, patmst.Patphoneno, patmst.IsActive,    "+
                  "  Deptwiseuser.username AS DeptUserName ,patmst.LabRegMediPro, SubDepartment.subdeptName, MainTest.Maintestname , patmstd.LabNo ,patmstd.MTCode, "+
                  "  patmstd.Patauthicante ,patmstd.ISReRun, patmstd.ReRunRemark ,patmstd.MachinName   "+
                  "  ,patmst.Isemergency,patmstd.PatientEmail,patmstd.IspheboAccept,patmst.PPID ,case when InterfaceStatus is null then 'No' else 'Yes' end as InterfaceStatus,patmstd.DoctorEmail  " +
                  "  , ISNULL(patmstd.SpecimanNo,0) as SpecimanNo , case when  ISNULL(patmstd.PanicResult,0) =0  then 'NA' else 'Panic' end  as PanicResult,patmstd.PanicRemark,patmstd.PanicInformToResult,patmstd.PanicAction,patmstd.InformUserName,Patmst.UploadPrescription,patmstd.Isoutsource,patmstd.OutsourcePatientPID,patmstd.OutResTransfer FROM         patmst INNER JOIN   " +
                  "  patmstd ON patmst.PID = patmstd.PID AND patmst.Branchid = patmstd.Branchid INNER JOIN   "+
                  "  MainTest ON patmstd.MTCode = MainTest.MTCode AND patmstd.Branchid = MainTest.Branchid INNER JOIN   "+
                  "  SubDepartment ON MainTest.SDCode = SubDepartment.SDCode AND MainTest.Branchid = SubDepartment.Branchid "+
                  "  INNER JOIN     "+              
                  "  Deptwiseuser ON SubDepartment.subdeptid = Deptwiseuser.DeptCodeID  " +
             ...
             if (CenterCodeNew != "")
             {
                 sc.CommandText = sc.CommandText + " and " + " patmst.CenterCode ='" + CenterCodeNew + "'";
             }
             if (Barid != "")
             {
             ...
             if (usertype == "Main Doctor" || usertype == "MainDoctor" || usertype == "Technician" || usertype == "Reporting")
             {
                 ...
                 {
                     // sql = sql + " and " + " VW_patlbvw.Username='" + username + "'";
                     sc.CommandText = sc.CommandText + " and " + "  Deptwiseuser.username='" + username + "'";
             ...
             sc.CommandText = sc.CommandText + "GROUP BY patmst.IsActive  , patmstd.TestDeActive  , patmst.PatRegID, CAST(patmst.Tests AS varchar(255)), patmst.PID, patmst.FID, patmst.Patregdate, patmst.intial, patmst.Patname, patmst.sex, patmst.Age,  " +
               " patmst.RefDr, patmst.Reportdate, patmst.DoctorCode, patmst.CenterCode, patmst.FinancialYearID, patmst.TestCharges, patmst.Username, patmst.SampleType,  "+
               " patmst.SampleStatus, patmst.RegistratonDateTime, patmstd.BarcodeID, patmst.Drname, patmst.MDY, CAST(patmst.PatientcHistory AS varchar(255)),  "+
               " patmst.Branchid, SubDepartment.DigModule, patmstd.UnitCode, patmst.Remark, patmst.Patphoneno, patmst.IsActive, SubDepartment.subdeptid,  "+
               " Deptwiseuser.username  ,patmst.LabRegMediPro , MainTest.Maintestname  ,patmstd.BarcodeID ,SubDepartment.subdeptName , patmstd.LabNo ,patmstd.MTCode,  "+
               " patmstd.Patauthicante,patmstd.ISReRun, patmstd.ReRunRemark,patmstd.MachinName ,patmst.Isemergency,patmstd.PatientEmail,patmstd.IspheboAccept, patmst.PPID,InterfaceStatus,patmstd.DoctorEmail,ISNULL(patmstd.SpecimanNo,0) ,ISNULL(patmstd.PanicResult,0) ,patmstd.PanicRemark,patmstd.PanicInformToResult,patmstd.PanicAction,patmstd.InformUserName,Patmst.UploadPrescription,patmstd.Isoutsource,patmstd.OutsourcePatientPID,patmstd.OutResTransfer ";
         ...
         {
             sc.CommandText = "Alter View VW_patlbvw as ( SELECT     patmst.PatRegID, CAST(patmstd.BarcodeID AS varchar(255)) AS Test1, patmst.PID, patmst.FID, patmst.Patregdate, patmst.intial, patmst.Patname, patmst.sex, "+
                           "  patmst.Age, patmst.RefDr, patmst.Reportdate, patmst.DoctorCode, patmst.CenterCode, patmst.FinancialYearID, patmst.TestCharges, patmst.Username,  "+
                           " patmst.SampleType, patmst.SampleStatus, patmst.RegistratonDateTime,  patmst.Drname, patmst.MDY, "+
                           " CAST(patmst.PatientcHistory AS varchar(255)) AS cliniclahist, patmst.Branchid, SubDepartment.DigModule, patmstd.UnitCode, patmst.Remark, patmst.Patphoneno,  "+
                           " patmst.IsActive, patmst.LabRegMediPro, SubDepartment.subdeptName, MainTest.Maintestname, patmstd.Labno, patmstd.MTCode, patmstd.Patauthicante, patmstd.ISReRun, "+
                           " patmstd.ReRunRemark, patmstd.MachinName, patmst.Isemergency, patmstd.PatientEmail ,patmstd.IspheboAccept, patmst.PPID,case when InterfaceStatus is null then 'No' else 'Yes' end as InterfaceStatus ,patmstd.DoctorEmail " +
                           " ,ISNULL(patmstd.SpecimanNo,0) as SpecimanNo,patmstd.BarcodeId,case when  ISNULL(patmstd.PanicResult,0) =0  then 'NA' else 'Panic' end  as PanicResult ,patmstd.PanicRemark,patmstd.PanicInformToResult,patmstd.PanicAction,patmstd.InformUserName,Patmst.UploadPrescription,patmstd.Isoutsource ,patmstd.OutsourcePatientPID,patmstd.OutResTransfer FROM         patmst INNER JOIN " +
                           " patmstd ON patmst.PID = patmstd.PID AND patmst.Branchid = patmstd.Branchid INNER JOIN "+
                           " MainTest ON patmstd.MTCode = MainTest.MTCode AND patmstd.Branchid = MainTest.Branchid INNER JOIN "+
                           " SubDepartment ON MainTest.SDCode = SubDepartment.SDCode AND MainTest.Branchid = SubDepartment.Branchid "+
             ...
             sc.CommandText = sc.CommandText + " GROUP BY patmst.IsActive  , patmstd.TestDeActive  , patmst.PatRegID, CAST(patmst.Tests AS varchar(255)), patmst.PID, patmst.FID, patmst.Patregdate, patmst.intial, patmst.Patname, patmst.sex, patmst.Age, " +
                     "   patmst.RefDr, patmst.Reportdate, patmst.DoctorCode, patmst.CenterCode, patmst.FinancialYearID, patmst.TestCharges, patmst.Username, patmst.SampleType,  "+
                     "   patmst.SampleStatus, patmst.RegistratonDateTime,  patmst.Drname, patmst.MDY, CAST(patmst.PatientcHistory AS varchar(255)), patmst.Branchid,  "+
                     "   SubDepartment.DigModule, patmstd.UnitCode, patmst.Remark, patmst.Patphoneno, patmst.IsActive, patmst.LabRegMediPro, SubDepartment.subdeptName, "+
                     ...
                     "   patmst.Isemergency, patmstd.PatientEmail ,patmstd.IspheboAccept, patmst.PPID,InterfaceStatus,patmstd.DoctorEmail ,ISNULL(patmstd.SpecimanNo,0),patmstd.BarcodeId ,ISNULL(patmstd.PanicResult,0) ,patmstd.PanicRemark,patmstd.PanicInformToResult,patmstd.PanicAction,patmstd.InformUserName,Patmst.UploadPrescription ,patmstd.Isoutsource ,patmstd.OutsourcePatientPID,patmstd.OutResTransfer " +
                                             " )";
         }
         try
         {
     ...
     public void AlterViewvw_VW_testmgr_nNew(object startDate, object endDate)
     {
         ...
         SqlCommand sc = conn.CreateCommand();
               ...
               SqlCommand cmd = new SqlCommand(" alter view VW_testmgr_n as SELECT DISTINCT "+
                  "  TOP (99.99) PERCENT '' AS MainDept, RTRIM(patmst.intial) + ' ' + patmst.Patname AS Name, patmst.PID, patmst.PatRegID, patmst.FID, patmst.intial, patmst.Patname, "+
                  ...
                  "  CAST(MainTest.Maintestname AS nvarchar(2000)) AS Testname, ISNULL(patmstd.SpecimanNo,0) as SpecimanNo ,PhlebotomistCollect,PhlebotomistRejectremark,OSBarcodeID " +
                  "  FROM         patmst INNER JOIN "+
    ...
    SqlCommand cmd = new SqlCommand(" alter view [VW_testmgr] as SELECT DISTINCT "+
           ...
           " Ocl.OutsourceLabName, patmst.LabRegMediPro, patmst.Isemergency, patmstd.IspheboAccept ,patmst.PPID ,ISNULL(patmstd.SpecimanNo,0) as SpecimanNo ,PhlebotomistCollect,PhlebotomistRejectremark,OSBarcodeID " +
           " FROM         patmst INNER JOIN "+
           " patmstd ON patmst.PID = patmstd.PID INNER JOIN "+
           " MainTest ON patmstd.MTCode = MainTest.MTCode AND patmstd.SDCode = MainTest.SDCode LEFT OUTER JOIN "+
           " Ocl ON patmstd.OutLabName = Ocl.Id  " +
     ...
     public static void AlterViewvw_VW_Result_Patmstd(string usertype, string Username, object startDate, object endDate)
     {
         ...
         sc.CommandText = "   Alter View VW_Result_Patmstd as    SELECT    * from patmstd " +
              " where  Patregdate >='" + DateTimeConvesion.getDateFromString(startDate.ToString()).ToString("dd/MMM/yyyy") + "' and Patregdate <'" + DateTimeConvesion.getDateFromString(endDate.ToString()).AddDays(15).ToString("dd/MMM/yyyy") + "'   ";
     ...
     public static void AlterViewvw_VW_Result_ResMst(string usertype, string Username, object startDate, object endDate)
     {
         ...
         sc.CommandText = "   Alter View VW_Result_ResMst as    SELECT    * from ResMst " +
              " where  ResDate >='" + DateTimeConvesion.getDateFromString(startDate.ToString()).ToString("dd/MMM/yyyy") + "' and ResDate <'" + DateTimeConvesion.getDateFromString(endDate.ToString()).AddDays(15).ToString("dd/MMM/yyyy") + "'   ";
     ...
     public void Update_rerun(string Barcode, string ReRunRemark, string PatRegID, string branchid, string MachinName)
     {
         SqlConnection conn = DataAccess.ConInitForPathology();
         SqlCommand sc = new SqlCommand("UPDATE patmstd SET patrepstatus=0 ,Patauthicante=@Patauthicante,ISReRun=@ISReRun,ReRunRemark=@ReRunRemark,MachinName=@MachinName where BarCodeID=@BarCodeID and PatRegID=@PatRegID and branchid='" + branchid + "'", conn);
     ...
     {
         SqlConnection conn = DataAccess.ConInitForPathology();
         string sql = "Select ''as TestStatus, * from VW_Printingpatientwise where FID='" + FID + "' and PatRegID<>'' and branchid=" + branchid + "";
     ...
     public DataSet ReportDownloadModify_Patientwise(object DoctorCode, object startDate, object endDate, object Patauthicante, string patientName, string Reg_no, int branchid, int DigModule, string subdeptName, string FID, string labcode_main, string Barcode, string Subdept, string RepStatus)
     {
         ...
         string sql = "Select  SrNo, PID, FID, PatRegID, Patregdate, PatientName, sex, Age, MDY, RefDr, Tests, PF, Reportdate, Phrecdate, flag, Patphoneno, Pataddress, Isemergency, Branchid, "+
          ...
          " from VW_Printingpatientwise where FID='" + FID + "' and PatRegID<>'' and branchid=" + branchid + " ";
