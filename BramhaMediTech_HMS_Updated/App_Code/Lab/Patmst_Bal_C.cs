...
public class Patmst_Bal_C
{
    ...
    public void getPermentCardId(string PatientCardNo)
    {
        ...
        SqlConnection conn = DataAccess.ConInitForPathology();
        SqlCommand sc = new SqlCommand("Select * from PatMT where PatientCardNo='" + PatientCardNo + "'", conn);
    ...
    {
        SqlConnection conn = DataAccess.ConInitForPathology();
        SqlCommand cmd = new SqlCommand("select DoctorCode from DrMT where DoctorCode='" + DrCode + "' and branchid=" + branchid + " and DrType='CC'", conn);
