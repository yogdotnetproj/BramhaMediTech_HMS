...
public class DrMT_sign_Bal_C
{
    ...
    public static ICollection Get_CenterDetails(string UnitCode, int branchid)
    {
        ...
        if (UnitCode != null)
        {
            sc = new SqlCommand("SELECT * FROM DrMT where DrType='CC' and UnitCode='" + UnitCode.ToString().Trim() + "' and branchid=" + branchid + " order by DoctorName", conn);
        }
        else
        {
    ...
    {
        SqlConnection conn = DataAccess.ConInitForPathology();
        SqlCommand cmd = new SqlCommand("select Cashbill  from DrMT where DoctorCode='" + DoctorCode + "'  and branchid=" + branchid + " and DrType='CC'", conn);
        object obj = null;
        bool cashflag = false;
        try
        {
    ...
    public static ICollection Get_CenterDetails(object labCode, int branchid)
    {
        ...
        if (labCode != null)
        {
            sc = new SqlCommand("SELECT * FROM DrMT where DrType='CC' and UnitCode='" + labCode.ToString().Trim() + "' and branchid=" + branchid + " order by DoctorName", conn);
        }
        else
        {
