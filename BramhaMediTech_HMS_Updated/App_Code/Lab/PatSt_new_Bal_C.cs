...
using System.Data.SqlClient;
    ...
    public class PatSt_new_Bal_C
    {
        ...
        public static ICollection GetPatst_Authorized_notingroup(string PatRegID, string FID, int branchid,int maindeptid,string subdept)
        {
            ...
            {
                if (maindeptid == 0)
                    sc = new SqlCommand("select * from Patmstd where PatRegID =@PatRegID and FID =@FID and Patauthicante = 'Authorized'  AND SDCode in (" + subdept + " ) and (PackageCode = '' ) and branchid=" + branchid + "  order by SDCode", conn);//and Patauthicante = 'Authorized'
                else
                    sc = new SqlCommand("select * from Patmstd where PatRegID =@PatRegID and FID =@FID and Patauthicante = 'Authorized'   AND SDCode in (" + subdept + " ) and (PackageCode = '' ) and branchid=" + branchid + " and SDCode in (select SDCode from SubDepartment where DigModule=" + maindeptid + ") order by SDCode", conn); //and Patauthicante = 'Authorized'
            }
            else
            {
                if (maindeptid == 0)
                    ...
                    sc = new SqlCommand("select * from Patmstd where PatRegID =@PatRegID and FID =@FID and Patauthicante = 'Authorized'  and (PackageCode = '' OR PackageCode is null) and branchid=" + branchid + " and SDCode in (select SDCode from SubDepartment where DigModule=" + maindeptid + ") order by SDCode", conn); //and Patauthicante = 'Authorized'
