...
public class calendars : System.Web.Services.WebService
{
    ...
    public void GetDocScheduleList(string docId)
    {
        ...
        DataTable dt = GetDocSchedule(docId);
        try
        {
            if (dt != null)
            {
    ...
    private DataTable GetDocSchedule(string docId)
    {
        DataTable dt = new DataTable();
        try
        {
                       ...
                       new SqlConnection(ConString))
            {
                ...
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@docId", SqlDbType.VarChar).Value = docId;
                    //cmd.Parameters.Add("@clickedDate", SqlDbType.VarChar).Value = string.IsNullOrEmpty(clickedDate) ? null : clickedDate;
    ...
    [WebMethod]
    public void SaveDocSchedule(string date1, string date2, string docId, string time1, string time2, string slotId)
    {
        try
        {
                       ...
                       new SqlConnection(ConString))
            {
                using (SqlCommand cmd = new SqlCommand("usp_InsertDocSchedule", connection))
                {
                    ...
                    cmd.Parameters.Add("@time1", SqlDbType.DateTime).Value = DateTime.ParseExact(time1, "HH:mm",
                                        CultureInfo.InvariantCulture);
                    cmd.Parameters.Add("@time2", SqlDbType.DateTime).Value = DateTime.ParseExact(time2, "HH:mm",
                                    CultureInfo.InvariantCulture);
